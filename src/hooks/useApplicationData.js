...
export default function useApplicationData() {
	...
	useEffect(() => {
				...
				for (const group of res) {
					getGroupMembers(group.id).then((data) => {
						setState((prev) => ({
							...prev,
							groups: res,
							group_members: { ...prev.group_members, data },
						}));
					});
				}
			})
		...
		getUser()
			...
			.then((res) => {
				return getPostedBills(res).then((posted) => {
	...
	const getUser = () => {
		return axios.get("/api/user").then((res) => res.data.id);
	};
	...
	const getUsersCurrentFriends = () => {
		return axios.get("/api/user/friends").then((res) => {
	...
	const createBill = (cost, description, group_id, includeSelf) => {
		...
		return axios.post("/api/bills", bill).then((res) => {
			setState((prev) => ({
				...prev,
	...
	const createGroup = (groupName, members) => {
		return axios
			.post("/api/groups", { name: groupName, members: members })
			.then((res) => {
				setState((prev) => ({
					...prev,
	...
	const payBill = (billID) => {
		return axios
			.post(`/api/bills/${billID}`, { paid: true })
			.then((res) => res.data);
	};

	const deleteBill = (billID) => {
		return axios.delete(`/api/bills/${billID}`).then((res) => res.data);
	};
	...
	const getPostedBills = (userID) => {
		return axios.get(`/api/bills/posted/${userID}`).then((res) => res.data);
	};

	const getReceivedBills = (userID) => {
		return axios.get(`/api/bills/received/${userID}`).then((res) => res.data);
	};

	const getGroupMembers = (groupID) => {
		return axios.get(`/api/groups/${groupID}`).then((res) => res.data);
	};
